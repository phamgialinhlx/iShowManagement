...
impl Platform {
    ...
    ///
    /// **Windows counts.** Not the OS itself — but zmux reaches a Windows host
...
/// rather than in either caller. `zmux-ssh` needs it to *reach* a Windows host
...
/// drift, and the failure would be a host zmux could connect to but not open a
/// terminal on.
///
