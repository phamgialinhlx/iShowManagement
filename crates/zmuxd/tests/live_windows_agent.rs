...
//!
//! This is the claim that matters: a shell started through zmux on Windows keeps
...
//!
//! ```text
//! ZMUX_LIVE_WINDOWS=my-windows-host cargo test -p zmuxd --test live_windows_agent -- --ignored --nocapture
//! ```

use std::path::PathBuf;

use zmuxd::provision::{self, DirectorySource};
use zmux_ssh::SshTarget;
use zmux_transport::{CommandSpec, Platform, SshHostId, Target, Tty};

fn live_host() -> Option<String> {
    std::env::var("ZMUX_LIVE_WINDOWS").ok().filter(|h| !h.is_empty())
}
...
#[tokio::test]
#[ignore = "needs a real Windows host; set ZMUX_LIVE_WINDOWS. \
...
async fn a_windows_session_outlives_its_connection() {
    let Some(host) = live_host() else {
        eprintln!("skipping: set ZMUX_LIVE_WINDOWS to a Windows host alias");
        return;
    };
    ...
    let _ = run(&target, "for p in \"$HOME\"/.zmux/bin/*.exe; do taskkill //F //IM \"$(basename \"$p\")\" >/dev/null 2>&1; done; rm -rf \"$HOME/.zmux\"").await;
...
async fn checks(target: &SshTarget) -> anyhow::Result<()> {
    ...
    // Killing the client is also a better model of what zmux does: quitting the
    // app kills the far end of the pipe, it does not politely close stdin.
    let probe = "/tmp/zmux-win-probe";
