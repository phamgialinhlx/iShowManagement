...
use zmux_transport::{CommandSpec, LocalTarget, Target, Tty, local::terminal_env};
...
struct Session {
    ...
    /// is zmux's convention and not the agent's business.
...
pub async fn serve() -> anyhow::Result<()> {
    ...
    tracing::info!(address = %endpoint.address, "zmuxd listening");
