...
//! network, quitting zmux — leaves the shell untouched on the far side.
//!
...
pub async fn attach(mut hello: Hello, start_daemon: bool) -> anyhow::Result<ExitCode> {
    ...
    //
    // Resizing zmux resizes the local PTY, and `ssh` passes that on to *this*
...
async fn wait_for_daemon(endpoint: &ipc::Endpoint) -> anyhow::Result<ipc::Stream> {
    ...
    anyhow::bail!("the zmuxd did not start listening at {}", endpoint.address)
}
...
fn sibling_binaries() -> Vec<std::path::PathBuf> {
    ...
    let mut found: Vec<std::path::PathBuf> = entries
        ...
        .filter(|p| {
            p.file_name()
                .and_then(|n| n.to_str())
                .is_some_and(|n| n.starts_with("zmuxd-"))
                && Some(p) != mine.as_ref()
        })
        .collect();
    found.sort();
...
/// answer on a host zmux has never touched — not an error to report.
pub async fn list() -> anyhow::Result<Vec<crate::protocol::SessionSummary>> {
    ...
    // separates "left behind" from "zmux is merely closed".
