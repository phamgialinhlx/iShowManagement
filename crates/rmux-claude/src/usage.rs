...
//!
//! Everything else rmux knows about spend is inferred from transcripts, which is
...
//
// Only the fields rmux uses are declared. The response gains fields between
...
/// The request rmux makes. Exposed so the URL can be asserted without a network.
