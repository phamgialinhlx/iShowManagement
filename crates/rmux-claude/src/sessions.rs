...
//! directory with every `/` replaced by `-`. Listing those files is how rmux
...
/// recourse is to start over. So rmux tries every spelling it knows, and falls
...
pub fn resolve_project_dir(folder: &str) -> String {
    let candidates = project_slug_candidates(folder)
        .iter()
        .map(|s| rmux_transport::shell_quote(s))
        .collect::<Vec<_>>()
        .join(" ");
    let folder = rmux_transport::shell_quote(folder.trim_end_matches('/'));
...
mod tests {
    ...
    fn a_folder_maps_to_claudes_project_directory() {
        ...
        assert_eq!(project_slug("/Users/macprom1/Code/rmux"), "-Users-macprom1-Code-rmux");
