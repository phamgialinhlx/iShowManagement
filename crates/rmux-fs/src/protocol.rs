...
//!
//! [`shell_quote_path`]: rmux_transport::shell_quote_path
//! [`shell_quote`]: rmux_transport::shell_quote
...
pub fn write_file_script(path: &str) -> String {
    ...
    format!(
        r#"p={quoted}
t=$(mktemp "${{TMPDIR:-/tmp}}/rmux.XXXXXX") || {{ printf 'E'; exit 1; }}
...
pub fn mkdir_script(path: &str) -> String {
    format!("mkdir -p -- {} && printf 'O'", rmux_transport::shell_quote_path(path))
}
...
pub fn delete_script(path: &str) -> String {
    format!("rm -rf -- {} && printf 'O'", rmux_transport::shell_quote_path(path))
}
...
pub fn rename_script(from: &str, to: &str) -> String {
    let from = rmux_transport::shell_quote_path(from);
    let to = rmux_transport::shell_quote_path(to);
    format!(
