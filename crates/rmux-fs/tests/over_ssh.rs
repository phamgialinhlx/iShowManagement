...
//! binary** on `PATH` that takes the final argument — the shell line rmux built —
...
//! behaviour. It proves the command rmux constructs is correct and that the far
//! side does the right thing with it.

use rmux_fs::{FileSystem, TargetFs};
...
fn install_fake_ssh(name: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("rmux-fakessh-{name}-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();

    let script = r#"#!/bin/sh
# Stand-in for the ssh binary. Options are ignored; the last argument is the
# shell line rmux built, which a real remote login shell would re-parse the same
...
fn workspace(name: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("rmux-ssh-fs-{name}-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    dir
}

/// Everything rmux does to a remote machine, over the ssh command path.
///
...
fn a_remote_machine_can_be_browsed_edited_and_measured() {
    ...
    runtime.block_on(async {
        ...
        match fs.read_file(&file).await.expect("read over ssh") {
            rmux_fs::FileContent::Text { text } => assert_eq!(text, "one\n"),
