...
//! work can swap the active theme at runtime (e.g. from `~/.zmux/settings.json`).
...
/// Register the settings provider and initialise the theme system. Call once
/// during app startup, after fonts are registered.
///
/// `theme::init` sets the active theme to Zed's own default — "One Dark"
/// (`fallback_themes::zed_default_dark`, the exact theme Zed itself ships
/// with) — and that is left in place rather than overridden with
/// `signal_room_theme` below, per the user's request to match Zed's default
/// look. `signal_room_theme` is kept (allow-dead-code) as the previous
/// default, in case switching back is wanted later.
pub fn init_theme(cx: &mut App) {
    ...
    // Sets up SystemAppearance, ThemeRegistry (loads "One Dark" as the active
    // theme via DEFAULT_DARK_THEME), FontFamilyCache, GlobalTheme.
    theme::init(LoadThemes::JustBase, cx);
}

#[allow(dead_code)]
fn use_signal_room_theme(cx: &mut App) {
