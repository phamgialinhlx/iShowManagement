//! Backend service layer: connect targets, provision `zmuxd`, list and
...
use zmuxd::provision::{self, DirectorySource, Installed};
use zmux_ssh::{SshTarget, config};
use zmux_transport::{
...
/// One row of `zmuxd list`, enriched with resource use and location.
...
///
/// The command is a **login line**, not a program name: zmux starts Claude as
/// `CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN=1 CLAUDE_CODE_DISABLE_MOUSE=1 claude`,
/// and a host commonly holds sessions started by an older zmux with a different
...
/// private `Update` (zmuxd/src/status.rs); the client defines its own.
...
impl Backend {
    ...
    /// A directory of `zmuxd-<triple>` binaries for development, before a
    /// bundle exists. Takes priority over the default layout.
    pub const AGENT_DIR_ENV: &'static str = "ZMUXD_DIR";
    ...
    /// Sessions `zmuxd` is holding on the host, enriched with resource
    /// use and cwd.
...
/// executable; a development build points `ZMUXD_DIR` at a directory of
/// `zmuxd-<triple>` binaries, or uses `target/agents` where
/// `scripts/build-agents.sh` writes them.
fn binary_source() -> DirectorySource {
    if let Ok(dir) = std::env::var(Backend::AGENT_DIR_ENV) {
        if !dir.is_empty() {
            let dir = PathBuf::from(&dir);
            let local = dir.join("zmuxd");
    ...
    if target_agents.exists() {
        let local = target_agents.join("zmuxd");
        if local.exists() {
...
/// Sessions `zmuxd` is holding on the host, with what they are consuming.
///
...
/// One row of `zmuxd list`, tolerant of both column counts.
///
...
async fn run_watcher(server: ReadyServer, mut sink: mpsc::UnboundedSender<StatusLine>, label: &str) {
    ...
    loop {
        ...
        log::info!(target: "zmux", "watch-status on {label}: reconnecting in {backoff:?}");
...
mod tests {
    ...
    /// Real rows from `zmuxd list` on a live host (six columns, `-` alias).
