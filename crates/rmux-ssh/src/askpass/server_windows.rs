//! The rmux side of the askpass bridge, on Windows.
//!
...
//! command rmux runs has neither. So it burns its retries against nothing and
...
//! its own connection and its own authentication: on a password host rmux
...
//! process *of the same user* phishing a credential dialog out of rmux.
...
impl AskpassServer {
    ...
    pub async fn start(answerer: Answerer) -> anyhow::Result<Self> {
        let name = format!(r"\\.\pipe\rmux-askpass-{}", short_id());
    ...
    /// string put into `RMUX_ASKPASS_SOCKET` for the helper to open.
...
fn create_instance(
    ...
    let server = unsafe {
        ServerOptions::new()
            ...
            // A named pipe is reachable over SMB unless this is set. rmux's
...
mod tests {
    ...
    async fn a_request_without_the_token_gets_nothing() {
        // Any local process of this user could otherwise ask rmux to pop a
    ...
    async fn a_second_prompt_does_not_wait_for_the_first() {
        ...
        // ERROR_PIPE_BUSY, which reads as "rmux ignored my password".
