...
//!
//! Every remote operation in zmux is a POSIX shell script — `zmux-fs` lists with
...
//! thing zmux does (`uname -s`) fails, so the connection never even completes.
//!
//! But a POSIX shell is almost always *present* — Git for Windows ships one, and
//! anyone using `ssh` to develop has Git. So zmux finds it and routes through it,
...
/// are even consulted. Routed through it, zmux would list the distro's files
...
///
/// So a host **with** WSL is the hazard here, not a host without one — zmux
...
///
/// Hand-rolled for the same reason `zmux-fs` hand-rolls it: forty lines of table
