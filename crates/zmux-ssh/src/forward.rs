...
//! here — that is the whole problem. zmux runs `ssh -N -L 3000:localhost:3000`
...
//!
//! **A forward must not be multiplexed.** zmux otherwise shares one connection
...
use zmux_transport::NoConsoleWindow;
...
impl Forwards {
    ...
    /// give each zmux session its own partition and proxy. zmux's own webview
