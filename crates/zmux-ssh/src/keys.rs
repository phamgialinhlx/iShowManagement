...
//! and zmux opens many: a terminal, a Claude session, a metrics sample, a file
...
use zmux_transport::{shell_quote, CommandSpec, NoConsoleWindow, Target};

/// Where zmux keeps the keys it generates.
///
/// Under `~/.ssh` rather than `~/.zmux`, because that is where `ssh` looks and
...
pub fn key_path(home: &Path, host: &str) -> PathBuf {
    ...
    // called `~/.ssh/zmux_a/b` would silently fail to be created.
    let safe: String = host
        .chars()
        .map(|c| if c.is_ascii_alphanumeric() || c == '-' || c == '.' { c } else { '_' })
        .collect();
    home.join(".ssh").join(format!("zmux_{safe}_ed25519"))
}
...
mod tests {
    ...
    fn the_key_is_quoted_into_the_script() {
        ...
        let script = install_script("ssh-ed25519 AAAAC3Nz zmux@my machine");
        assert!(script.contains("'ssh-ed25519 AAAAC3Nz zmux@my machine'"), "{script}");
    }
    ...
    fn the_file_is_appended_not_written() {
        let script = install_script("ssh-ed25519 AAAA zmux");
    ...
    fn a_host_alias_cannot_escape_its_filename() {
        ...
        assert_eq!(path, home.join(".ssh").join("zmux_.._.._etc_evil_ed25519"));
    ...
    fn ordinary_aliases_stay_readable() {
        ...
        assert!(path.ends_with("zmux_build-box.example.com_ed25519"));
    }
}
