...
//! terminal. zmux has no terminal to offer, so without intervention a
...
//! (`askpass/`) forwards the prompt to the running zmux instance over a Unix
...
fn env_for(config: Option<&AskpassConfig>) -> BTreeMap<String, String> {
    ...
    env.insert("ZMUX_ASKPASS_SOCKET".to_owned(), config.socket.display().to_string());
    ...
    env.insert("ZMUX_ASKPASS_ATTEMPT".to_owned(), uuid::Uuid::new_v4().simple().to_string());
    ...
    env.insert("ZMUX_ASKPASS_TOKEN".to_owned(), config.token.clone());
...
pub struct Prompt {
    ...
    pub message: String,
    pub kind: PromptKind,
    /// Which `ssh` invocation asked, from `ZMUX_ASKPASS_ATTEMPT`.
    ///
...
mod tests {
    ...
    fn installing_forces_ssh_to_use_the_helper() {
        let config = AskpassConfig {
            helper: PathBuf::from("/opt/zmux/askpass"),
            socket: PathBuf::from("/tmp/zmux.sock"),
            token: "tok-123".to_owned(),
        };
        let env = env_for(Some(&config));

        assert_eq!(env.get("SSH_ASKPASS").map(String::as_str), Some("/opt/zmux/askpass"));
        ...
        assert_eq!(env.get("ZMUX_ASKPASS_SOCKET").map(String::as_str), Some("/tmp/zmux.sock"));
        // Without this the helper cannot prove it is ours and gets no answer.
        assert_eq!(env.get("ZMUX_ASKPASS_TOKEN").map(String::as_str), Some("tok-123"));
    }
    ...
    fn registering_a_helper_is_visible_to_the_whole_process() {
        ...
        assert!(!is_installed());
        install(
            PathBuf::from("/opt/zmux/askpass"),
            PathBuf::from("/tmp/zmux.sock"),
            "tok-123".to_owned(),
        );
        assert!(is_installed());
        assert_eq!(
            env_for_gui_prompts().get("SSH_ASKPASS").map(String::as_str),
            Some("/opt/zmux/askpass")
        );
        uninstall();
        assert!(!is_installed());
    }
