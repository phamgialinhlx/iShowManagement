...
//! `ssh` on a remote host. zmux watches the rendered screen and answers it.
//!
...
use zmux_transport::{CommandSpec, Target, Tty};
...
/// ruinous, and all three of the symptoms zmux shipped with came from it:
///
...
pub enum Rendering {
    /// Appended to the terminal's own scrollback. zmux's default.
    #[default]
    Inline,
...
impl Rendering {
    ...
    /// through the agent's `Hello` instead (see `zmuxd`), because
...
impl ClaudeSession {
    ...
    /// The shell line zmux runs to launch Claude.
    ///
    /// Exposed so the caller can hand it to the agent instead of running it
    /// directly — which is what makes a conversation keep working after zmux is
    /// closed.
    pub fn launch_line(resume: Option<&str>, args: &[String], rendering: Rendering) -> String {
        ...
        if let Some(id) = resume {
            ...
            launch.push_str(&zmux_transport::shell_quote(id));
        }
        for arg in args {
            launch.push(' ');
            launch.push_str(&zmux_transport::shell_quote(arg));
        }
        launch
    }
    ...
    /// zmux can simply read where each one belongs and set the folder itself.
...
mod launch_tests {
    ...
    fn inline_is_the_default() {
        ...
        // preference forever, so zmux has to opt out on every launch.
    ...
    fn the_launch_shell_reads_rc_files() {
        ...
        let target = zmux_transport::LocalTarget::new();
...
mod tests {
    use super::*;
    use zmux_transport::LocalTarget;
