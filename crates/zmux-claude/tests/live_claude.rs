...
//!
//! ```text
//! ZMUX_LIVE_HOST=SingaporeDev cargo test -p zmux-claude --test live_claude \
//!   -- --ignored --nocapture
//! ```

use zmux_claude::ClaudeSession;
use zmux_ssh::SshTarget;
use zmux_term::TermSize;
use zmux_transport::{SshHostId, Target};
...
#[ignore = "needs a real SSH host with claude installed; set ZMUX_LIVE_HOST"]
async fn claude_launches_on_a_real_remote_host() {
    let Some(host) = live_host() else {
        eprintln!("skipping: set ZMUX_LIVE_HOST");
        return;
    };
    ...
    let out = target
        .exec(
            &zmux_transport::CommandSpec::new("$SHELL")
                .arg("-l")
                .arg("-c")
                .arg("claude --version")
                .tty(zmux_transport::Tty::None),
        )
        .await
        .expect("exec");
...
async fn a_real_claude_dialog_is_parsed() {
    ...
    let folder = format!("~/zmux-dialog-probe-{}", std::process::id());
    target
        .exec(
            &zmux_transport::CommandSpec::new("sh")
                .arg("-c")
