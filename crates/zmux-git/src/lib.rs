...
//!
//! Same reasoning as `zmux-fs::search`, and it matters more here. Answering
...
use zmux_transport::{shell_quote, CommandSpec, Target};
use serde::Serialize;
...
const START: &str = "__ZMUX_GIT_BEGIN_9f2c7d__";
...
const SPLIT: &str = "__ZMUX_GIT_SPLIT_9f2c7d__";
...
mod tests {
    ...
    const STATUS: &str = concat!(
        ...
        "# branch.head zmux\x00",
        "# branch.upstream origin/zmux\x00",
        "# branch.ab +2 -1\x00",
    ...
    fn the_branch_and_its_divergence_are_read() {
        ...
        assert_eq!(s.branch, "zmux");
        assert_eq!(s.upstream.as_deref(), Some("origin/zmux"));
    ...
    fn the_real_thing_parses() {
        ...
        assert_eq!(s.changes[2].path, "crates/zmux-git/");
