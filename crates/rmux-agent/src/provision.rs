...
//!
//! rmux runs `rmux-agent attach` *on the target* rather than a login shell. That
//! is what makes a terminal survive: the shell belongs to a daemon the
//! connection cannot reach, so quitting rmux, losing the network or closing the
//! laptop leaves it running. Reattaching is by **name**, so the same terminal tab
//! comes back to the same shell across restarts.
//!
//! For a local target the binary already exists next to rmux. For an SSH target
//! it has to be put there once, which is what this module does.

use std::path::{Path, PathBuf};

use rmux_transport::{CommandSpec, Target, Tty, shell_quote};
...
pub fn triple_for(platform: &RemotePlatform) -> anyhow::Result<&'static str> {
    ...
    anyhow::ensure!(
        os == "linux",
        "no prebuilt rmux-agent for {} — persistent sessions need one",
        platform.os
    );
...
///
/// Version-stamped, so upgrading rmux installs alongside rather than over — a
...
pub fn prune_script(home: &str, keep: &str) -> String {
    let bin = shell_quote(&format!("{}/.rmux/bin", home.trim_end_matches('/')));
    let keep = shell_quote(keep);
    format!(
        r#"for f in {bin}/rmux-agent-*; do
  [ -f "$f" ] || continue
  [ "$f" = {keep} ] && continue
...
/// the target is a binary rmux owns entirely, and the risk being defended against
...
pub fn install_script_for(
    ...
    format!(
        ...
        bin = shell_quote(&format!("{home}/.rmux/bin")),
        dir = shell_quote(&format!("{home}/.rmux")),
        tmp = tmp,
...
pub async fn ensure<T: Target + ?Sized>(target: &T, binaries: &dyn BinarySource) -> anyhow::Result<Installed> {
    // Local: the agent ships beside rmux, so there is nothing to install.
    if matches!(target.id(), rmux_transport::TargetId::Local)
    ...
    anyhow::ensure!(
        out.status == 0,
        "could not install the rmux agent: {}",
        out.stderr.trim()
    );
...
/// Reads binaries from a directory of `rmux-agent-<triple>` files.
...
impl BinarySource for DirectorySource {
    fn agent_for(&self, triple: &str) -> anyhow::Result<Vec<u8>> {
        let path = self.dir.join(format!("rmux-agent-{triple}"));
...
pub fn default_source(resource_dir: Option<&Path>, exe_dir: Option<&Path>) -> DirectorySource {
    let local_name = if cfg!(windows) { "rmux-agent.exe" } else { "rmux-agent" };
...
mod tests {
    ...
    fn the_install_path_is_version_stamped() {
        // Upgrading rmux must not overwrite the binary a running daemon came
    ...
    fn pruning_leaves_binaries_with_a_running_daemon() {
        ...
        let script = prune_script("/home/x", "/home/x/.rmux/bin/rmux-agent-1-a");
    ...
    fn pruning_only_touches_agent_binaries() {
        ...
        let script = prune_script("/home/x", "keep");
        assert!(script.contains("rmux-agent-*"), "{script}");
    ...
    fn attach_passes_the_session_name_and_size() {
        let installed = Installed { program: "/opt/rmux-agent".into() };
        let spec = installed.attach_spec("term-7", Some("/srv/app"), 120, 40);

        assert_eq!(spec.program, "/opt/rmux-agent");
    ...
    fn reattaching_without_a_cwd_does_not_move_the_shell() {
        ...
        let installed = Installed { program: "rmux-agent".into() };
