...
//!
//! ```text
//! RMUX_LIVE_HOST=SingaporeDev cargo test -p rmux-agent --test live_provision -- --ignored --nocapture
//! ```

use std::path::PathBuf;

use rmux_agent::provision::{self, BinarySource, DirectorySource};
use rmux_ssh::SshTarget;
use rmux_transport::{CommandSpec, SshHostId, Target, Tty};
...
async fn the_agent_installs_and_keeps_a_shell_alive() {
    ...
    let _ = run(&target, "pkill -f rmux-agent; rm -rf \"$HOME/.rmux\"").await;
...
async fn checks(target: &SshTarget) -> anyhow::Result<()> {
    ...
    let mode = run(target, "stat -c %a \"$HOME/.rmux\"").await?;
    anyhow::ensure!(
        mode.trim() == "700",
    ...
    let before = run(target, "stat -c %Y \"$HOME/.rmux/bin\"/rmux-agent-* 2>/dev/null").await?;
    let again = provision::ensure(target, &binaries).await?;
    let after = run(target, "stat -c %Y \"$HOME/.rmux/bin\"/rmux-agent-* 2>/dev/null").await?;
...
async fn login_shell_checks(target: &SshTarget, agent: &str) -> anyhow::Result<()> {
    let probe = "/tmp/rmux-live-login-probe";
    let _ = run(target, &format!("rm -f {probe}")).await;
    ...
    let start = format!(
        "nohup {agent} attach --session login-probe --cwd /tmp \
         ...
         sleep 4",
        agent = agent,
        line = rmux_transport::shell_quote(&line),
    );
    run(target, &start).await?;
    ...
    anyhow::ensure!(
        alive.trim() != "0",
        "the session died with its client — Claude would stop the moment rmux closed"
    );
...
/// rmux's xterm and the shell the daemon owns. Left cooked it echoes what it
...
async fn raw_mode_checks(target: &SshTarget, agent: &str) -> anyhow::Result<()> {
    let probe = "/tmp/rmux-live-raw-probe";
    let _ = run(target, &format!("rm -f {probe}")).await;
