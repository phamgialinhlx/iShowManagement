...
//!
//! zmux reaches a Windows host through Git for Windows' bash, because OpenSSH
//! for Windows hands the remote command to `cmd.exe` and every zmux operation is
...
//!
//! ```text
//! ZMUX_LIVE_WINDOWS=my-windows-host cargo test -p zmux-fs --test live_windows -- --ignored --nocapture
//! ```

use zmux_fs::{FileContent, FileSystem, SearchQuery, TargetFs};
...
async fn a_windows_host_behaves_like_any_other() {
    ...
    let root = format!("{home}/zmux-live-windows");
...
async fn checks(fs: &dyn FileSystem, root: &str) -> anyhow::Result<()> {
    ...
    match fs.read_preview(&binary).await? {
        zmux_fs::PreviewContent::Base64 { bytes: n, .. } => {
