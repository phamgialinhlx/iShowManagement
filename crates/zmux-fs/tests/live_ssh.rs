...
//! proves the command zmux builds is correct, but it cannot prove authentication,
...
//!
//! ```text
//! ZMUX_LIVE_HOST=SingaporeDev cargo test -p zmux-fs --test live_ssh -- --ignored --nocapture
//! ```
//!
...
use zmux_fs::{FileContent, FileSystem, TargetFs};
...
async fn a_real_host_can_be_browsed_edited_and_measured() {
    ...
    let root = format!("{home}/zmux-live-test");
...
async fn run_checks(fs: &dyn FileSystem, root: &str) -> anyhow::Result<()> {
    ...
    match fs.read_preview(&dropped).await? {
        zmux_fs::PreviewContent::Base64 { bytes, base64 } => {
            anyhow::ensure!(
    ...
    match fs.read_preview(&dropped).await? {
        zmux_fs::PreviewContent::Base64 { bytes, .. } => anyhow::ensure!(
            bytes == payload.len() as u64,
...
async fn the_folder_browser_can_navigate_a_real_host() {
    ...
    let folders: Vec<&zmux_fs::DirEntry> = entries
        .iter()
        .filter(|e| e.kind == zmux_fs::EntryKind::Directory)
        .collect();
...
async fn previews_encode_identically_on_both_targets() {
    ...
    let dir = std::env::temp_dir().join(format!("zmux-b64-{}", std::process::id()));
    ...
    let zmux_fs::PreviewContent::Base64 { base64: ours, bytes: our_len } = local_preview else {
...
async fn compare_encodings(
    ...
    let encoded_path = format!("{home}/zmux-b64-probe.b64");
    let binary_path = format!("{home}/zmux-b64-probe.bin");
    ...
    let result = async {
        remote
            .run_for_test(&format!(
                "base64 -d {} > {} || base64 -D {} > {}",
                zmux_transport::shell_quote(&encoded_path),
                zmux_transport::shell_quote(&binary_path),
                zmux_transport::shell_quote(&encoded_path),
                zmux_transport::shell_quote(&binary_path),
            ))
            .await?;
        ...
        let zmux_fs::PreviewContent::Base64 { base64: theirs, bytes: their_len } = preview else {
...
async fn a_real_file_downloads_byte_for_byte() {
    ...
    let fs = TargetFs::new(SshTarget::new(SshHostId::new(&host)));
    let home = fs.home_dir().await.expect("home_dir");
    let root = format!("{home}/zmux-download-test");
    let _ = fs.delete(&root).await;
    ...
    let dir = std::env::temp_dir().join("zmux-download-check");
    let _ = std::fs::remove_dir_all(&dir);
