...
//! which is what lets zmux be an ordinary local IDE and a remote one with the
//! same editor.
//!
...
use zmux_transport::{CommandSpec, NoConsoleWindow, Target, Tty};
...
mod tests {
    ...
    fn temp_dir(name: &str) -> std::path::PathBuf {
        let dir =
            std::env::temp_dir().join(format!("zmux-fs-{name}-{}", std::process::id()));
    ...
    async fn a_missing_file_reports_an_error() {
        let err = LocalFs::new().read_file("/nonexistent/zmux/nope.txt").await.unwrap_err();
