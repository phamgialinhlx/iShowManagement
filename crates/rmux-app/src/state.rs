//! `~/.rmux/state.json` — which servers were connected and which session names
...
/// A session kind. Shells are `rmux-agent` sessions running a login shell;
...
fn path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".rmux").join("state.json"))
}
...
impl State {
    ...
    pub fn load() -> Self {
        ...
        match std::fs::read_to_string(&path) {
            Ok(text) => match serde_json::from_str::<Self>(&text) {
                Ok(state) => state,
                Err(e) => {
                    log::warn!(target: "rmux", "state.json unreadable, starting fresh: {e}");
                    Self::default()
                }
            },
            Err(_) => Self::default(),
    ...
    pub fn save(&self) {
        ...
        if let Err(e) = result {
            log::warn!(target: "rmux", "state.json save failed: {e}");
