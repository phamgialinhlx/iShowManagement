...
//!
//! rmux is a GUI process, and a GUI process on Windows has **no console**. When
...
//!
//! On its own that would be ugly. What makes it unusable is how often rmux
//! spawns `ssh`: Windows OpenSSH has no `ControlMaster` (see
//! [`crate::Platform`] and `rmux_ssh::mux`), so **every** command opens its own
